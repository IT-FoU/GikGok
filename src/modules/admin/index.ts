...
export function filterAuditRows<
  T extends { action?: string; action_type?: string; target_type?: string | null },
>(rows: T[], filters: { action?: string; targetType?: string }): T[] {
  return rows.filter((row) => {
    const actionText = (row.action_type ?? row.action ?? "").toLowerCase();
    if (filters.action && !actionText.includes(filters.action.toLowerCase())) {
      return false;
    }
