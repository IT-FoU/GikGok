...
export function EngineBetForm({
  ...
  const stakeId = useId();
  const [stake, setStake] = useState(definition.quickStakes[0] ?? definition.minStake);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[var(--brand-muted)]">
        Server-authoritative engine stub. Outcomes and payouts are computed only
        on the server. Balance: {balance.toLocaleString()} GIK.
      </p>
