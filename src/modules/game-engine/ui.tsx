...
export function AdminGameControls() {
  ...
  const [gameId, setGameId] = useState<GameId>("fish_prawn_crab");
  ...
  function defaultPayload(id: GameId): string {
    if (id === "high_low") return '{"dice":[1,2,3]}';
    if (id === "spinning_plate") return '{"landedSlot":12}';
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="rounded-md border border-[var(--brand-border)] p-4">
        <summary className="cursor-pointer font-medium">
          Controlled demo setup (must be set before the round)
        </summary>
        <form
        ...
        >
          ...
          <fieldset className="space-y-2">
            <legend className="text-sm text-[var(--brand-muted)]">
              Open next round
            </legend>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          ...
          <input
            type="hidden"
            name="message"
            value="Game temporarily unavailable."
          />
