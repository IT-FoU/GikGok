...
import { PlateGameClient } from "@/modules/games/spinning-plate/plate-game";
import { createServerSupabaseClient } from "@/lib/supabase/server";

export const dynamic = "force-dynamic";

export default async function SpinningPlatePage() {
  let balance = 0;
  let graphicsMode: "auto" | "2d" | "3d" = "auto";
  let graphicsQuality: "low" | "medium" | "high" = "medium";
  let history: Array<{
    id: string;
    stake: number;
    is_win: boolean;
  ...
  try {
    ...
    graphicsQuality =
      (settings?.graphics_quality as "low" | "medium" | "high") ?? "medium";
    history = (receipts ?? []).map((row) => ({
      ...
      selection: (row.selection ?? {}) as { slot?: number },
