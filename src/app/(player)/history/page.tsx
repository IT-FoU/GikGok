...
export default async function HistoryPage({
  ...
  const { data: receipts } = await supabase
    .from("bet_receipts")
    .select(
      "id, bet_id, game_id, game_version_id, settlement_mode, stake, selection, total_return_multiplier, payout_amount, balance_after, is_win, created_at",
    )
    ...
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6">
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--brand-muted)]">
          Server-settled bet receipts. Demo credits only.
        </p>
      </div>
