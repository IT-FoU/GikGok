...
describe("High–Low settlement matrix", () => {
  ...
  it("rejects invalid side and insufficient balance", () => {
    ...
    expect(buildHighLowSelection("both")).toMatchObject({ error: expect.any(String) });
    expect(
      validateBetRequest({
        gameId: "high-low",
        stake: 5000,
...
describe("High–Low receipt parse and recovery", () => {
  it("parses server payload and replay for refresh recovery", () => {
    ...
    expect(parseHighLowServerResult({ game: "fish-prawn-crab" })).toBeNull();
  });
